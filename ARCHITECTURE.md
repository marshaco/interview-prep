...
Scope × pace × finish date: the user fixes scope plus **exactly one** of pace or finish date — a strict mode split, not two simultaneously-editable fields — and the engine derives the third.

- **Scope** — an explicit `ModuleId[]` (`engine/plan/scope.ts`'s `PlanScope`), picked from a mini-map in setup (§14.5), not a `'all'`/ancestor-implying sentinel. `resolveScopeModuleIds` resolves the list to exactly the modules that are both selected and authored — no automatic ancestor expansion: selecting a module whose prerequisites aren't also selected is allowed (nothing is gated), it just means those prerequisites contribute nothing to the projection. "Everything" is simply the list of every authored module id at setup time, and — because it's a literal list, not a live `'all'` — it does **not** grow when new content is authored later; the user edits the plan to add it.
- **Mode** — `PlanPaceInput = { mode: 'pace'; minutesPerDay } | { mode: 'date'; targetDate }` (`storage/types.ts`), a discriminated union so there's no representable state with both or neither set. Whichever isn't stored is always derived at read time via `deriveThird` (`engine/plan/resolvePlanInputs.ts`'s `resolvePlanInputs`, the one function every consumer — the hero's plan line, plan details — funnels through, so the day list and the finish date always come from the same resolved pace). `activeDays` (7 booleans, Sun–Sat, matching `Date#getDay()`) applies to either mode. Pace presets are 15/30/45/60/90 plus a `Custom` chip that reveals a numeric field (floor 10) only when chosen — never chips and a spinner shown at once.
...
- **`projectPlan`** simulates day by day from `now`: the in-scope "new content" backlog is built by `engine/plan/workItems.ts`'s `remainingWorkItems` — literally re-running `selectNextAction` against a snapshot that incrementally "completes" each item, rather than a second ordering policy that could drift from the one Home's hero already uses. Each active day spends its budget on due reviews first (most overdue first, at review cost), then new backlog items (at practice cost) until the budget runs out; an inactive day still advances the simulated calendar by one day, it just spends zero budget, so a due review on a rest day simply stays due for the next active day rather than being skipped or lost. Every simulated solve is assumed a clean, not-fast pass; if the exercise is `reviewable`, it enters the **same** pure scheduler (`enterReview`/`scheduleReview`, §13.2) used everywhere else — future review load is *projected* by running the real scheduler forward, never separately estimated. Output includes `finishDateIso` (when the new-content backlog empties — inclusive of `now`'s own calendar day, so a plan whose whole backlog fits in today's budget finishes today, not tomorrow; `null` if not reached within `horizonDays`), a full `dailyLoad[]` (one entry per calendar day, active or not), and `totalRemainingMinutes` (new content only — reviews are perpetual and have no finite total). Budget comparisons use a small epsilon tolerance (`BUDGET_EPSILON`) since a review's `estimatedMinutes * 0.6` cost isn't always exactly representable in floating point (e.g. `18 * 0.6 === 10.799999999999999`) — without it, an item that should fit a budget exactly could be wrongly deferred a day.
...
- **`todayTarget`** is `projectPlan` with `horizonDays: 1`, plus `minutesSpentToday` computed from real attempt durations on the current calendar day (distinct from the simulation's own bookkeeping — the hero's plan line progress bar tracks actual time spent, not simulated cost).
...
- **Correctness is locked with hand-computed fixtures, not just properties** (`projectPlan.test.ts`'s "fixture scenarios" block) — property tests alone weren't sufficient to catch date-arithmetic bugs, so a handful of scenarios assert a literal expected calendar date computed by hand: a fixed-size non-reviewable backlog finishing on an exact day count; the same with a weekend skipped, starting on a known weekday; reviewable content's finish date measurably later than the naive `total ÷ budget` estimate; and a pace→date→pace round trip agreeing within one active day.
...
export interface PlanRecord {
  scope: ModuleId[]; // explicit list — no 'all' sentinel; not auto-updated when new content is authored
  pace: PlanPaceInput; // exactly one of pace or date; the other is always derived, never stored
...
A single record (`plan` Dexie table, keyed at a fixed id since there is only ever one row) via `StorageAdapter.getPlan`/`savePlan`/`deletePlan` — no daily-completion table; day progress is derived from `attempts` timestamps, never stored. Included in `ExportBundleV1` (nullable — older exports predate it). Two migrations getting here from the original shape: `v6` collapses flat `minutesPerDay`/`targetDate` fields into the `pace` discriminated union (a set `targetDate` becomes date-mode, otherwise pace-mode from the existing `minutesPerDay`); `v7` converts `scope` from `'all' | ModuleId` into the explicit list — `'all'` migrates to every currently-authored module id, a specific module id migrates to that module plus its full ancestor closure (preserving the plan's actual effective coverage rather than silently shrinking it to just the one id). Both upgrade functions live in `storage/dexie/db.ts` and duplicate small amounts of engine logic locally (ghost-module filtering, ancestor closure) since `storage/` sits below `engine/` in the layer order and can't import from it.
...
- **Setup** (`PlanSetupDialog`, a centered dialog): a scope mini-map (below), a `Daily time | Finish date` mode switch above the pace/date area (exactly one editable; the other renders as a read-only derived line — `Finish: {date}` in pace mode, `{n} min/day` in date mode — switching modes converts the currently-derived value into the new mode's input rather than discarding it), and weekday toggles (filled accent circle = selected, outline + muted text = unselected — legible as a pattern at a glance, not seven identical circles). Pace is one control: preset chips (15/30/45/60/90) plus a `Custom` chip that reveals the numeric field only when chosen, never chips and a spinner shown together. Infeasibility renders as plain text next to a disabled `Start plan` (`Needs 180+ min/day — extend the date or narrow the scope` in date mode) — never a disabled control with no explanation. No `~`/"about" anywhere; displayed minutes round to the nearest 5 (`ui/components/plan/planFormat.ts`'s `roundMinutes`).
  - **Scope mini-map** (`PlanScopeMap`): the same tiered DAG layout Home uses, at reduced scale, as toggle chips (selected = accent border + check glyph, unauthored = recessed and inert — never toggleable). `All`/`None` plain-text links above; a live `{n} modules · {n} exercises · {H}h {M}m total` summary below. Selecting a module whose prerequisites aren't also selected produces one quiet informational line (`Note: {module} builds on {unselected prerequisites}`) — never a blocking or warning-colored state.
- **Hero merge:** there is no separate plan strip. When a plan is active (and not paused), the Home hero card gains a second line below the CTA: `Today: {n} reviews · {n} new · {n} min` (plain text, secondary color) plus a thin real-progress bar (`minutesSpentToday / minutesPerDay`) and a right-aligned status (`On track — finish {date}` / `Finish moved to {date}`, the latter only when a user-set `targetDate` exists and the live projection has slipped past it). Rest days and done-for-today both render with dry, check-glyph-only copy ("Rest day · next: …", "Done for today") — the words "behind"/"missed"/"overdue" and any red/warning color never appear, by design. The CTA (line 1, the card's one accent element) is unchanged by the plan; clicking the plan line (not the CTA) opens plan details — the two are separate clickable regions in the same card, never nested. With no plan (or a paused one), the hero is single-line as always, and the quiet entry link sits below it ("Set up a study plan →", or "Resume plan →" when paused — resuming is immediate, no dialog).
- **Details** (`PlanDetailsDialog`, opened by clicking the hero's plan line): every calendar day through the 7th active one, inclusive of interleaved rest days shown inline and muted (`Thu 24 · 2 reviews · 3 new · 25 min`, `Sat 26 · rest`) — generated from the same `projectPlan` call the finish date comes from, so the list and the date can never disagree — plus the projected finish date and scope summary. `Edit` reopens setup prefilled; `Pause` sets `pausedAt` (collapses to the entry-link slot, now reading `Resume plan →`); `Delete` (confirm-then-remove) drops the record entirely.
