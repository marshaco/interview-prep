 ...
 * (harness.test.ts runs under Vitest).
 */
...
/**
 * Builds the Python harness source for a function-mode HarnessSpec.
 *
 * The harness owns its own ListNode class and only ever reads results back
 * via duck typing (getattr for .val/.next) — it never assumes the user's
 * code left ListNode intact, so a renamed or broken user ListNode class
 * can't break grading of inputs the harness itself builds.
 *
...
export function buildHarness(spec: HarnessSpec): string {
  ...
  const argTypesB64 = toBase64Json(spec.argTypes);
  const resultTypeB64 = toBase64Json(spec.resultType);
  const testsB64 = toBase64Json(spec.tests);

  return `import base64, json, random, sys
    ...
    if not callable(_fn):
        ...
        for t in _TESTS:
            ...
            try:
                ...
                cmp_fn = _COMPARATORS[t['comparator']]
                entry['passed'] = bool(cmp_fn(got, t['expected']))
                if t['group'] == 'visible':
                    entry['args'] = t['args']
